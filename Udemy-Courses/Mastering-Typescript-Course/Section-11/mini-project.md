...
```
- Inside the context of the `addEventListener()` args, TypeScript is able to determine that `e`, the event has a method of `preventDefault` even though it wasnt directly typed, and is no better than a generic `e` anywhere else.
